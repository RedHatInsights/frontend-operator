...
function start_builder() {
    ...
    if [ -z "$BUILDER_EXISTS" ]; then
        ...
        docker --config="$DOCKER_CONF" buildx create --name multiarch --platform linux/amd64,linux/arm64 --use --driver-opt image=quay.io/domino/buildkit:latest --driver-opt network=host --buildkitd-flags '--allow-insecure-entitlement network .host'
    fi
