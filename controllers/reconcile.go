...
func (r *FrontendReconciliation) run() error {
	...
	if r.Frontend.Spec.Image != "" {
		...
		// If reverse proxy is enabled for the environment, add the reverse proxy deployment
		if r.FrontendEnvironment.Spec.EnableReverseProxy && r.FrontendEnvironment.Spec.ReverseProxyImage != "" && r.Frontend.Spec.ReverseProxyEnabled {
...
func (r *FrontendReconciliation) createReverseProxyDeployment() error {
	...
	deploymentName := r.Frontend.Name + "-reverse-proxy"
